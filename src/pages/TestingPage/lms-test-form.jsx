...
import { addLmsFlow, addTestingOption, getLmsFlows, getTestingOptions } from '../../services/dropdownApi';
...
export default function LmsTestForm() {
  ...
  const loadModuleFlows = useCallback(async (module) => {
    ...
    try {
      const response = await getLmsFlows({ limit: 10000, offset: 0, module: normalizedModule });
  ...
  const onFinish = async (values) => {
    ...
    setSubmitting(true);
    try {
      await addLmsFlow(payload);
      message.success('LMS case details saved successfully.');
      form.resetFields();
      setFlowRows([]);
      setFlowOptions([]);
    } catch (error) {
      console.error('Unable to save LMS case:', error);
      message.error('Unable to save LMS case.');
    } finally {
      setSubmitting(false);
    }
  };
...
<Row align="middle" justify="space-between" className="breadcrumb-action-wrapper">
  ...
  <Col>
    <Button type="link" onClick={() => navigate('/lms-home')}>
      View Table
    </Button>
  </Col>
</Row>
        ...
        <div className="v1-page-inner">
          <div className="v1-page-header">
            <Space align="center" size={10}>
              ...
              <div>
                <Text strong style={{ fontSize: 16 }}>Add LMS Case</Text>
              </div>
            </Space>
          </div>
          ...
          <AppCard>
            ...
            >
              <Card
                className="v1-card"
                title={<SectionTitle icon={<FileTextOutlined />} title="LMS Case Details" />}
                extra={
